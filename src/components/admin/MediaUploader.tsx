            ...
            {images.map(m => (
              <div key={m.id as string} className="relative group">
                ...
                <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={() => setPrincipal(m.id as string)} className={`p-1 rounded ${m.es_principal ? 'bg-primary text-white' : 'bg-white text-gray-600'} shadow`}>
                    <Star className="w-4 h-4" />
                  </button>
                  <button onClick={() => deleteMedia(m.id as string)} className="p-1 rounded bg-white text-red-600 shadow">
                    <X className="w-4 h-4" />
                  </button>
                </div>
                {m.es_principal ? <span className="absolute bottom-2 left-2 bg-primary text-white text-xs px-2 py-0.5 rounded">Principal</span> : null}
              </div>
            ))}
          </div>
        )}
      ...
      <div>
        ...
        {videos.length > 0 && (
          <div className="space-y-4 mb-4">
            {videos.map(v => (
              <div key={v.id as string} className="flex items-center gap-4 bg-gray-50 rounded-lg p-3">
                ...
                <p className="text-sm text-gray-600 truncate flex-1">{v.url as string}</p>
                <button onClick={() => deleteMedia(v.id as string)} className="text-red-600 hover:text-red-800 p-1">
                  <X className="w-4 h-4" />
                </button>
              </div>
            ))}
        ...
        <div className="flex gap-2">
          ...
          />
          <button onClick={addVideoUrl} className="bg-primary hover:bg-primary-dark text-white px-4 py-2 rounded-lg flex items-center gap-2">
